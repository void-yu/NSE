...
def build_labeled_sequence(seq, class_label, weight_type='Unrestricted'):
    """Builds labeled sequence from input sequence.
    Args:
        seq: SequenceWrapper.
        class_label: bool.
        weight_type: str.
            If 'Linear', class_label will be put on every timestep and weight will increase linearly from 0 to 1.
            Elif 'Last', class_label will be put on every timestep and weight will only be put on last timestep and is set to 1.
            Elif 'Uniform', class_label and weight will be put on every timestep equally and sum of weight is 1.
            Elif 'Unrestricted', class_label and weight will be put on every timestep equally and each weight is 1.
            Else 'NoLabel', class_label will not be put on every timestep and weight will only be put on last timestep and is set to 1.
    ...
    for i, timestep in enumerate(seq):
        ...
        if weight_type == 'Linear':
            ...
            weight = 1.0 if seq_len < MIN_LENGTH else float(i) / (seq_len - 1)
            label_timestep.set_weight(weight)
        elif weight_type == 'Last':
            if i == (seq_len - 1):
                label_timestep.set_label(int(class_label))
                label_timestep.set_weight(1.)
            else:
                label_timestep.set_label(int(class_label))
                label_timestep.set_weight(0.)
        elif weight_type == 'Uniform':
            label_timestep.set_label(int(class_label))
            label_timestep.set_weight(1. / (seq_len - 1))
        elif weight_type == 'Unrestricted':
            label_timestep.set_label(int(class_label))
            label_timestep.set_weight(1.)
        else:
            if i == (seq_len - 1):
