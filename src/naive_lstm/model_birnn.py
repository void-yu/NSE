...
class TextRNN:
    def __init__(self, num_classes, learning_rate, batch_size, decay_steps, decay_rate, sequence_length,
                 vocab_size, embed_size, is_training, pretrained_wv, initializer=tf.random_normal_initializer(stddev=0.1)):
        ...
        self.input_y = tf.placeholder(tf.float32, [None], name="input_y")  # y [None,num_classes]
        ...
        self.loss_scalar = tf.summary.scalar('loss', self.loss_val)

        self.train_op = self.train()
        # self.predictions = tf.argmax(self.logits, axis=1, name="predictions")  # shape:[None,]
        self.accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.round(tf.sigmoid(self.logits)), tf.reshape(self.input_y, [-1, 1])), tf.float32), name="Accuracy") # shape=()
        self.acc_scalar = tf.summary.scalar('acc', self.accuracy)

    def instantiate_weights(self):
        """define all weights here"""
        with tf.name_scope("embedding"): # embedding matrix
            # self.Embedding = tf.Variable(self.pretrained_wv, name='embeddings', dtype=tf.float32)
            self.Embedding = tf.constant(self.pretrained_wv, name='embeddings', dtype=tf.float32)
            del self.pretrained_wv
    ...
    def loss(self, l2_lambda=0.0001):
        with tf.name_scope("loss"):
            ...
            losses = tf.nn.sigmoid_cross_entropy_with_logits(logits=self.logits, labels=tf.reshape(self.input_y, [-1, 1]))
...
#     PRETRAINED_VECS_PATH = 'D://Codes/NSE/data/used/embeddings/word-picked.vec'
#     def get_vecs():
#         vecs = []
#         words = []
...
#     textRNN = TextRNN(num_classes, learning_rate, batch_size, decay_steps, decay_rate, sequence_length, vocab_size, embed_size, is_training, np.array(pretrained_wv))
