...
def find_random_PCA_senti_directions(vecs, num=40, topk=1):
    indexs = np.random.choice(64189, num)
    matrix = vecs[indexs]
    print(np.shape(matrix))
    pca = PCA(n_components=10)
    pca.fit(matrix)
...
dire_randomPCA_list = find_random_PCA_senti_directions(turned_vecs, num=100, topk=3) # score: 0.74
# dire_lda = p_n_lda()
